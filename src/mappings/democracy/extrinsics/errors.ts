...
export function TooManyOpenVotes(block: number, wallet?: string) {
    return `Each wallet can only have one active vote at a time. 2 or more votes are active for wallet ${wallet} at block ${block}`
}

export function NoOpenVoteFound(block: number, wallet?: string) {
    return `No active vote found for wallet ${wallet} at block ${block}`
}